class Team < ActiveRecord::Base
  ...
  def team_average_time(test_id)
    sql = "select to_char(to_timestamp(avg(s.value)) AT TIME ZONE 'UTC +04:30','HH24:MI:SS') as average from scores s, teams_users ts where s.test_id = #{test_id} and s.user_id = ts.user_id and ts.team_id = #{id};"
